...
public class SecurityConfig {
  ...
  /**
   * Set up a security filter chain and configure the access levels for the different endpoints. We
   * then configure application into a "Resource Server" that accepts "JWT"s
   *
   * <p>You can always do a request to /token to have a look at how Spring Security has parsed the
   * given token.
   *
   * @param httpSecurity
  ...
  // custom validator
  /**
   * You can write a custom validation by adding a {@see JwtClaimValidator} for instance below we
   * add a custom validator to the aud (audience) claim. And check that it contains a certain
   * string. {@see OAuth2TokenIntrospectionClaimNames} contains static string names of several
   * default claims. Below we are referencing the {@see OAuth2TokenIntrospectionClaimNames.AUD}
   * string.
   *
